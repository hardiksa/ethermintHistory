...
SIMAPP = ./app
HTTPS_GIT := https://github.com/hardiksa/ethermint.git
...
docker-build:
	...
	docker create --name ethermint -t -i hardiksa/ethermint:latest ethermint
...
PACKAGE_NAME:=github.com/hardiksa/ethermint
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/hardiksa/ethermint/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/hardiksa/ethermint/tests/importer
